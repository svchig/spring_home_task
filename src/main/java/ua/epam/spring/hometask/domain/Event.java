...
public class Event extends DomainObject {
    ...
    public Event(long id, String name, NavigableSet<LocalDateTime> airDates, double basePrice, EventRating rating) {
        super(id);
        this.name = name;
    ...
    public boolean airsOnDates(LocalDateTime from, LocalDateTime to) {
        return airDates.stream()
                .anyMatch(dt -> dt.toLocalDate().compareTo(ChronoLocalDate.from(from)) >= 0 && dt.toLocalDate().compareTo(ChronoLocalDate.from(to)) <= 0);
    }
